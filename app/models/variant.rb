class Variant < ActiveRecord::Base
  ...
  def widget_header
    attr = super
    attr.present? ? attr : DEFAULT_WIDGET_HEADER
  end

  def widget_solicit
    attr = super
    attr.present? ? attr : DEFAULT_WIDGET_SOLICIT
  end

  def widget_follow_up_cta
    attr = super
    attr.present? ? attr : DEFAULT_WIDGET_CTA
  end

  def widget_thanks
    attr = super
    attr.present? ? attr : DEFAULT_WIDGET_THANKS
  end

  def banner_thanks
    attr = super
    attr.present? ? attr : DEFAULT_BANNER_THANKS
  end

  def email_body
    attr = super
    attr.present? ? attr : DEFAULT_EMAIL_BODY
  end
