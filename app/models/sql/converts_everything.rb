  ...
  class ConvertsEverything
    ...
    def convert
      ::User.all.no_timeout.each do |mongo_user|
        @converts_object.convert(mongo_user, Sql::User)
      end

      ::Campaign.all.no_timeout.each do |c|
        if Sql::Campaign.find_by(:mongo_id => c.id.to_s).present?
          ...
          @converts_object.convert(c, Sql::Campaign)
